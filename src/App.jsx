...
:root{--bg:#F8F7F3;--white:#fff;--ink:#17160F;--ink2:#3D3C35;--gold:#B8912A;--gold2:#D4A843;--gold-bg:rgba(184,145,42,0.08);--gold-border:rgba(184,145,42,0.25);--muted:#8A8980;--border:#E5E4DC;--surface:#F1F0EB;--green:#1E4035;--red:#8B1A1A;--shadow:0 2px 12px rgba(23,22,15,0.08);--shadow-lg:0 8px 40px rgba(23,22,15,0.13);}
...
.nb:hover{background:var(--surface)}.nb.on{background:var(--gold-bg);color:var(--gold);font-weight:500}
...
.chip:hover{border-color:var(--gold);color:var(--ink)}.chip.on{background:var(--gold);border-color:var(--gold);color:#fff;font-weight:500}
...
.btn2:hover{border-color:var(--gold);color:var(--ink)}.btn2.red:hover{border-color:var(--red);color:var(--red)}
...
.ring::before{animation:spin 1s linear infinite}.ring::after{animation:spin 0.7s linear infinite reverse;inset:7px;border-top-color:var(--gold2)}
...
.phrase:last-child{border-bottom:none}.phrase:hover{background:var(--surface);color:var(--ink)}
...
.live-ref{font-size:16px;color:var(--gold);letter-spacing:0.12em;text-transform:uppercase;margin-bottom:20px}
...
.toast{position:fixed;bottom:28px;left:50%;transform:translateX(-50%);background:var(--ink);color:#fff;padding:10px 20px;border-radius:100px;font-size:13px;z-index:500;animation:toastIn 0.2s ease forwards}
...
@media(max-width:680px){.sidebar{display:none}.main{margin-left:0}.page{padding:16px 14px 60px}.obody{padding:18px}}
`;
async function ai(system, user, tokens = 2200) {
  const r = await fetch("/api/chat", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:tokens,system,messages:[{role:"user",content:user}]}) });
...
function Suggestions({ onSelect }) {
  ...
  return <div className="chips">{list.map(([t,r])=>(<button key={t} className="chip" onClick={()=>onSelect(t,r)}>{t}</button>))}</div>;
}
function LiveMode({ sermon, onClose }) {
  return (
    <div className="live-overlay" onClick={onClose}>
      ...
      <div className="live-hint">Clique para sair do modo pregação</div>
    </div>
  );
}
function SermonGen({ saves, setSaves, showToast }) {
  ...
  const styles = ["Evangelístico","Expositivo","Devocional","Profético","Temático","Para jovens"];
  async function generate() {
    if (!topic) return;
    setLoading(true); setSermon(null); setPrayer(""); setPhrases([]);
    try {
      const sys = PS + `\n\nFormato de saída JSON:\n{"title":"...","ref":"...","text":"sermão completo aqui com introdução, 3 pontos, conclusão e apelo","prayer":"oração de fechamento","phrases":["frase1","frase2","frase3","frase4","frase5"]}`;
      const prompt = `Gere um sermão cristocêntrico completo sobre: ${topic}${ref ? ` (referência: ${ref})` : ""}. Duração: ${dur} minutos. Estilo: ${style.length ? style.join(", ") : "balanceado"}. O sermão deve ter introdução impactante, 3 pontos exegéticos, aplicação prática e apelo ao Evangelho.`;
    ...
    <div className="page">
      <div className="f" style={{display:"grid",gap:14}}>
        <div style={{display:"grid",gap:4}}>
          ...
          <input value={topic} onChange={e=>setTopic(e.target.value)} onKeyDown={e=>e.key==="Enter"&&generate()} placeholder="Ex: Ansiedade, João 3.16, Propósito de vida..." />
        </div>
        <Suggestions onSelect={(t,r)=>{setTopic(t);setRef(r);}} />
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <div style={{display:"grid",gap:4}}>
            <label>Versículo base (opcional)</label>
        ...
        <button className="btn" onClick={generate} disabled={!topic||loading}>
          {loading?<><div className="dots"><span/><span/><span/></div>Construindo sermão cristocêntrico...</>:"✦ Gerar Sermão"}
        </button>
      </div>
      ...
      {sermon && !loading && (
        <div className="out">
          ...
          {prayer && (<div className="prayer" style={{marginTop:14}}><div className="prayer-label">🙏 Oração de Abertura</div><div className="prayer-text">{prayer}</div></div>)}
          <div className="obody">{sermon.text}</div>
          {phrases.length > 0 && (
            <div className="pbox">
              <div className="pbox-title">✦ Frases de Impacto — Clique para copiar</div>
              {phrases.map((p,i)=>(<div key={i} className="phrase" onClick={()=>{navigator.clipboard.writeText(`"${p}"`);showToast("Frase copiada!");}}>{p}</div>))}
  ...
  return (
    <div className="page">
      <div className="f" style={{display:"grid",gap:14}}>
        ...
        <button className="btn" onClick={gen} disabled={!cat||loading}>{loading?<><div className="dots"><span/><span/><span/></div>Buscando...</>:"✦ Sugerir Temas"}</button>
      </div>
      {loading && <div className="loader"><div className="ring"/><p>Buscando temas...</p></div>}
      {items.length>0 && !loading && (
        <div className="out" style={{display:"grid",gap:10}}>
...
function VerseSearch({ showToast }) {
  ...
  async function search() {
    ...
    try {
      const r = await aiJSON(PS, `Encontre 5 versículos NVI mais relevantes para: "${q}". Formato: [{"ref":"...","text":"texto completo","why":"por que se aplica"}]`, 900);
      if(r) setRes(r);
    } catch(e) { showToast("Erro: "+e.message); }
    setLoading(false);
  }
  return (
    <div className="page">
      <div className="f" style={{display:"grid",gap:12}}>
        ...
        <button className="btn" onClick={search} disabled={!q||loading}>{loading?<><div className="dots"><span/><span/><span/></div>Buscando...</>:"✦ Buscar"}</button>
      </div>
        ...
        <div className="out" style={{display:"grid",gap:10}}>
          {res.map((v,i)=>(
            <div key={i} style={{background:"var(--surface)",borderRadius:10,padding:"14px 16px",border:"1px solid var(--border)"}}>
              ...
              <button className="btn2" style={{marginTop:8}} onClick={()=>{navigator.clipboard.writeText(`${v.ref} — ${v.text}`);showToast("Copiado!");}}>📋 Copiar</button>
            </div>
          ))}
        </div>
      )}
...
function Outline({ showToast }) {
  ...
  async function gen() {
    ...
    try {
      const r = await aiJSON(PS, `Crie um esboço completo de sermão sobre: ${topic}. Formato: {"title":"...","ref":"...","intro":"...","points":[{"title":"...","text":"...","verse":"..."}],"conclusion":"...","appeal":"..."}`, 1200);
      if(r) setOutline(r);
    } catch(e) { showToast("Erro: "+e.message); }
    setLoading(false);
  }
  return (
    <div className="page">
      <div className="f" style={{display:"grid",gap:12}}>
        <div style={{display:"grid",gap:4}}><label>Tema do esboço</label>
          <input value={topic} onChange={e=>setTopic(e.target.value)} onKeyDown={e=>e.key==="Enter"&&gen()} placeholder="Ex: A graça que transforma, João 3.16..."/>
        </div>
        <button className="btn" onClick={gen} disabled={!topic||loading}>{loading?<><div className="dots"><span/><span/><span/></div>Criando...</>:"✦ Criar Esboço"}</button>
      </div>
      {loading && <div className="loader"><div className="ring"/><p>Criando esboço...</p></div>}
      {outline && !loading && (
        <div className="out">
          <div className="otag">✦ Esboço</div>
          ...
          <div className="brow">
            <button className="btn2" onClick={()=>{const t=`${outline.title}\n${outline.ref||""}\n\nIntrodução:\n${outline.intro}\n\n${outline.points?.map((p,i)=>`${i+1}. ${p.title}\n${p.text}\n(${p.verse})`).join("\n\n")}\n\nConclusão:\n${outline.conclusion}\n\nApelo:\n${outline.appeal}`;navigator.clipboard.writeText(t);showToast("Copiado!");}}>📋 Copiar Esboço</button>
          </div>
        </div>
      )}
    </div>
        ...
        <aside className="sidebar">
          ...
          <div style={{padding:"10px 18px 12px",borderBottom:"1px solid var(--border)",background:"var(--surface)"}}>
            ...
            <div style={{fontSize:14,fontWeight:700,color:"var(--ink)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <button onClick={()=>{supabase.auth.signOut();localStorage.removeItem("pregar-user");setUser(null);}} style={{fontSize:9,color:"var(--muted)",background:"none",border:"none",cursor:"pointer",padding:0,opacity:0.6}}>sair</button>
            </div>
          </div>
          <div className="sb-body">
            <div className="sb-sec">Menu</div>
            {nav.map(n=>(<button key={n.id} className={`nb${page===n.id?" on":""}`} onClick={()=>{setPage(n.id);setSelSave(null);}}><span className="nb-ic">{n.ic}</span>{n.label}</button>))}
          </div>
          {saves.length>0 && (
            <div className="sb-saves">
              <div className="sb-saves-title">Salvos</div>
              {saves.map(s=>(<div key={s.id} className="save-item" onClick={()=>{setSelSave(s);setPage("saved");}}><span className="save-item-t">{s.title}</span><button className="save-del" onClick={e=>{e.stopPropagation();handleSaveSaves(saves.filter(x=>x.id!==s.id));showToast("Removido");}}>✕</button></div>))}
            </div>
          )}
        </aside>
        <main className="main">
          ...
          {page==="saved" && selSave && (
            <div className="page"><div className="out">
